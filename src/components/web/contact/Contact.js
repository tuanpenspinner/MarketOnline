...
const Contact = () => {
  ...
  const resetForm = () => {
    setContact({ content: "", email: "", name: "", phone: "", address: "" });
    setValidated(false);
  };
          ...
          <Form noValidate validated={validated} onSubmit={sendContactSubmit} className="mt-3">
            <Form.Row>
              <Form.Group as={Col} md="6">
                <Form.Control required type="text" name="name" pattern="^(?!\s*$).+" placeholder="Họ tên" onChange={onChange} value={contact.name} />
              ...
              <Form.Group as={Col} md="6">
                <Form.Control required name="address" pattern="^(?!\s*$).+" placeholder="Địa chỉ" onChange={onChange} value={contact.address} />
